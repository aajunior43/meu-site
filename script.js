...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== INTERATIVIDADE DA SEÇÃO DE SERVIÇOS =====
...
function initServicesInteractivity() {
    ...
    const cardObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry, index) => {
            if (entry.isIntersecting) {
                setTimeout(() => {
                    entry.target.style.animation = `serviceCardFadeIn 0.6s ease forwards`;
    ...
    serviceCards.forEach((card, index) => {
        ...
        card.addEventListener('mouseenter', function() {
            ...
            // Efeito de pulse no preço
            const priceValue = this.querySelector('.price-value');
            if (priceValue) {
                priceValue.style.animation = 'pricePulse 0.5s ease';
            }
        });
        ...
        // Efeito de clique para destacar temporariamente
        card.addEventListener('click', function(e) {
            if (e.target.tagName !== 'A') {
                ...
                this.style.boxShadow = '0 0 50px rgba(0, 212, 255, 0.5)';
    ...
    // Função para animar preços
    function animatePrice(element) {
        const text = element.textContent;
        const isHourly = text.includes('/h');
        const isMonthly = text.includes('/mês');
        const numberMatch = text.match(/[\d.,]+/);

        if (numberMatch) {
            const targetValue = parseFloat(numberMatch[0].replace('.', '').replace(',', '.'));
            ...
            const timer = setInterval(() => {
                ...
                let displayValue;
                if (targetValue >= 1000) {
                    displayValue = Math.floor(currentValue).toLocaleString('pt-BR');
                } else {
    ...
    // Adiciona CSS das animações dinamicamente
    const serviceAnimationCSS = `
    ...
    styleSheet.textContent = serviceAnimationCSS;
    document.head.appendChild(styleSheet);

    // Efeito parallax sutil no fundo da seção
    ...
    if (servicesSection) {
        window.addEventListener('scroll', () => {
            ...
            if (scrolled + windowHeight > sectionTop && scrolled < sectionTop + sectionHeight) {
                ...
                servicesSection.style.backgroundPosition = `center ${parallaxValue}px`;
            }
        });
    }
...
function initSearchSystem() {
    ...
    // Dados para busca (simulado)
    const searchData = [
        {
            title: 'Desenvolvimento Web',
            excerpt: 'Sites modernos, responsivos e otimizados para conversão.',
            url: 'index.html#services',
            type: 'serviço'
        },
        {
            title: 'Aplicativos Mobile',
            excerpt: 'Apps nativos e híbridos para iOS e Android.',
            url: 'index.html#services',
            type: 'serviço'
        },
        {
            title: 'Sobre Mim',
            excerpt: 'Conheça minha trajetória, habilidades e experiências.',
            url: 'about.html',
            type: 'página'
        },
        {
            title: 'Portfólio',
            excerpt: 'Explore meus projetos e trabalhos realizados.',
            url: 'portfolio.html',
            type: 'página'
        },
        {
            title: 'Contato',
            excerpt: 'Entre em contato para conversar sobre seu projeto.',
            url: 'contact.html',
            type: 'página'
        },
        {
            title: 'Sistemas Personalizados',
            excerpt: 'ERPs, CRMs e sistemas sob medida.',
            url: 'index.html#services',
            type: 'serviço'
        },
        {
            title: 'Blog',
            excerpt: 'Artigos sobre tecnologia e desenvolvimento.',
            url: 'blog.html',
            type: 'página'
        }
    ];
        ...
        if (results.length === 0) {
            searchResults.innerHTML = `
                <div class="search-result-item">
                    ...
                    <div class="search-result-excerpt">Tente usar palavras-chave diferentes.</div>
                </div>
            `;
        } else {
    ...
    function highlightText(text, query) {
        ...
        return text.replace(regex, '<span style="background: var(--accent-color); color: #000; padding: 2px 4px; border-radius: 3px;">$1</span>');
    }
